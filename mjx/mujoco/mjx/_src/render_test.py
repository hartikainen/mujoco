...
class RenderTest(parameterized.TestCase):
  ...
  def test_pmap_render_context(self):
    ...
    dx, (depth, rgb) = jax.pmap(reset)(key)
    self.assertEqual(depth.shape, (n_devices, nworld, m.ncam, width * height))
    self.assertEqual(rgb.shape, (n_devices, nworld, m.ncam, width * height))
    for i in range(2):
      dx, (depth, rgb) = jax.pmap(step)(dx)
      self.assertEqual(depth.shape, (n_devices, nworld, m.ncam, width * height))
      self.assertEqual(rgb.shape, (n_devices, nworld, m.ncam, width * height))
