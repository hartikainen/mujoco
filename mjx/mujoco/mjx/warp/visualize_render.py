...
_USE_PMAP = flags.DEFINE_boolean(
    'use_pmap', False, 'enable pmap'
)
...
def render_pipeline(m, mx, worldids, rc):
  ...
  dx_batch = init_fn(worldids)

  # Forward
  dx_batch = jax.vmap(forward.forward, in_axes=(None, 0))(mx, dx_batch)

  # Refit BVH
  dx_batch = jax.vmap(warp_bvh.refit_bvh, in_axes=(None, 0, None))(
      mx, dx_batch, rc
  )
  ...
  # Render
  out_batch = jax.vmap(warp_render.render, in_axes=(None, 0, None))(
      mx, dx_batch, rc
  )
...
def _main(_: Sequence[str]):
  ...
  if _USE_PMAP.value:
    ...
    rgb = jax.vmap(
      render_util.get_rgb,
      in_axes=(None, 0, None),
    )(rc_wrapper, rgb_packed, _CAMERA_ID.value)
    depth = jax.vmap(
      functools.partial(render_util.get_depth, depth_scale=4.0),
      in_axes=(None, 0, None),
    )(rc_wrapper, depth_packed, _CAMERA_ID.value)
  ...
  rgb = jax.vmap(render_util.get_rgb, in_axes=(None, 0, None))(
    rc, rgb_packed, _CAMERA_ID.value
  )
  depth = jax.vmap(
    functools.partial(render_util.get_depth, depth_scale=4.0),
    in_axes=(None, 0, None),
  )(rc, depth_packed, _CAMERA_ID.value)
